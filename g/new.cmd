...
for /f "tokens=2,3,5,7,* delims='`] " %%i in ('call s ls') do (
  echo %%i %%j %%k
  if "%%i" == "??" call :toadd %%j
  if "%%i" == "RM" call :todel %%j
  if "%%i" == "R" call :todel %%j
  if "%%i" == "AD" call :todel %%j
)
...
:doaddfiles
if "%addfiles%" == "" (echo Nothing to add.   ) else (%VCS_CMD% add %addfiles%)
goto :EOF

:dodelfiles
if "%delfiles%" == "" (echo Nothing to delete.) else (%VCS_CMD% add --all %delfiles%)
goto :EOF
